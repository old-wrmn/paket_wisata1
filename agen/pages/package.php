			...
			<div class="card mt-2">
				...
				<div class="card-body">
					<div class="alert">
						...
						<strong>Danger!</strong> No workship place
					</div>
					<div class="form-group row">
				...
				<div class="card-footer d-flex justify-content-between align-items-center bg-info border-top-0">
					...
					<a type="button" class="btn btn-outline bg-white text-white border-white border-2" id="go" onclick="cekMosq('<?php echo $idmax ?>')">Save</a>

					<!-- 		        <?php
											$selesai = mysqli_query($conn, "SELECT COUNT(id_worship_place) as msq from object_point where id_package='$idmax'");
											$baris = mysqli_fetch_array($selesai);
											$msq = $baris['msq'];
											if ($msq == 0) {
											?>
		        <button type="button" class="btn btn-outline bg-danger text-white border-white border-2" data-toggle="modal" data-target="#modal_theme_danger">SAVE</button>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<h6 class="font-weight-semibold">There is no Worship Place on this Package</h6>
				</div>

				<div class="modal-footer">
					<button type="button" class="btn btn-link" data-dismiss="modal">Add</button>
					<button type="button" class="btn bg-danger" onclick="delPackage('<?php echo $idmax ?>')">Discard Package</button>
				</div>
			</div>
		</div>
	</div>
...
<script type="text/javascript">
	...
	function cariObjek() {
		...
		if (objek != 0 && district != 0) {
			$.ajax({
				url: "http://localhost/paket_wisata/agen/act/cariObjek.php?objek=" + objek + "&district=" + district,
				data: "",
				dataType: 'json',
	...
	function show1() {
		...
		if (document.getElementById('name').value != "" & document.getElementById('price').value != "") {
			...
			$.ajax({
				url: "act/savePackage.php?data1=1&data2=" + id + "&data3=" + id_agen + "&data4=" + name + "&data5=" + price + "&data6=" + itinerary,
				data: "",
				dataType: 'json',
				success: function(rows) {
	...
	function cekMosq(id_package) {
		console.log("act/cekMosq.php?id_package=" + id_package);
		$.ajax({
			url: "act/cekMosq.php?id_package=" + id_package,
			type: "GET",
			dataType: 'json',
			data: {
				msq: "msq"
			...
			success: function(data) {
				console.log(data[0].msq);
				console.log(data);
				let msq = data[0].msq;
				console.log(msq);
				if (msq == 0) {
					console.log($('#modal_theme_danger'));
					$('.alert').show();
				} else {
