							...
							<tr>
                                ...
                                <?php 
                                	if ($status=="PERLU DIKIRIM") {
                                		$pd = "BELUM DIPESAN";
                                		echo "<td style=\"text-align: center; width: 15%\"><span class=\"badge badge-warning\" style=\"font-size:12px\"> $pd <span></td>";
                                	}elseif ($status=="TERKIRIM") {
                                		$terkirim = "SUDAH DIPESAN";
                                		echo "<td style=\"text-align: center; width: 15%\"><span class=\"badge badge-success\" style=\"font-size:12px\"> $terkirim <span></td>";
                                	}elseif ($status=="SELESAI") {
                                		echo "<td style=\"text-align: center; width: 15%\"><span class=\"badge badge-info\" style=\"font-size:12px\"> $status <span></td>";
                                	}
                                ?>
                                <td style="text-align: center; width: 15%">
                                    <div class="btn-group">

                						<button a onclick="hapus('<?=$id_booking?>')" <?php if ($data['status']=="SELESAI") { echo "disabled";  }  ?> id="btndel" class="btn btn-outline bg-danger-400 text-danger-400 border-danger-400 mr-1" title='Delete'><i class="icon-trash"></i></button>
                						
                						<button a onclick="selesai('<?=$id_booking?>')" <?php if ($data['status']=="PERLU DIKIRIM" || $data['status']=="SELESAI") { echo "disabled";  }   ?> id="btnsend" class="btn btn-outline bg-info-400 text-info-400 border-info-400" title='Completed'><i class="icon-checkmark4"></i></button>
                                    </div>
                                </td>
							</tr>
						<?php } ?>
